...
def draw_menu(
    ...
    for particle in particles:
        ...
        if v_pos[1] + 10 < r.bottom and text_rect.centery > r.top:
            if not particle.vel.length() == 0:
                pygame.draw.line(
                    ...
                    (255, 255, 255),
                    v_pos,
                    v_pos + (particle.vel.normalize() * 10),
                    2,
                )
                pygame.draw.line(
                    screen,
                    (0, 255, 0),
                    v_pos,
                    v_pos + (particle.acc.normalize() * 10),
                    2,
                )
        ...
        if (
            trace_mode
            and text_rect.centery + 10 < r.bottom
            and text_rect.centery > r.top
        ):
            pygame.draw.line(
                screen,
                (255, 255, 255),
                (text_rect.left - 8, text_rect.centery),
                particle.pos,
            )
...
def draw_cmd(
    ...
    if cmd_mode:
        pygame.draw.rect(
            screen, (75, 82, 81), pygame.Rect(r.left, r.top, r.width, r.height)
        )
    else:
        pygame.draw.rect(
            screen, (45, 52, 61), pygame.Rect(r.left, r.top, r.width, r.height)
        )
